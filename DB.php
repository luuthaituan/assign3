<?php
 class DB
 {
     ...
     public function writeLog($executeTime, $sql){
        ...
        if(!file_exists(dirname(__DIR__).'/assign3/logs/mysql')){
            mkdir(dirname(__DIR__).'/assign3/logs/mysql', 0777);
        }
        $path = dirname(__DIR__).'/assign3/logs/mysql/'.$query.date('Ymd_His').'.log';
 ...
 $db->getData();
 //$db->updateData(2 ,['title' => "bai 3454", 'content' => "noi dung 3454" ]);
//$db->deleteData(6);
 //$db->insertData(['title' => "bai 3454", 'content' => "noi dung 3454" ]);